...
export default function TreesAll() {
  ...
  const contributionGroups = useMemo<ContributionGroup[]>(() => {
    ...
    return Object.entries(grouped).map(([contribId, rows]) => {
      ...
      const partnerId = first.plantation_partner_id;
      return {
  ...
  const filtered = useMemo(() => {
    let result = contributionGroups.filter(g => {
      const matchSearch = !search ||
        ...
        g.partner_name?.toLowerCase().includes(search.toLowerCase());
              ...
              <>
                <SheetHeader>
                  <SheetTitle className="flex items-center gap-2">
                    <TreePine className="h-5 w-5 text-admin-primary" />
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="rounded-lg border bg-card p-4">
                      <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
                        ...
                        <div>
                          <span className="text-muted-foreground text-xs">Plantation Partner</span>
                          <p className="font-medium">{viewSheet.partner_name || "Unassigned"}</p>
                        </div>
                        <div className="text-right">
                          <span className="text-muted-foreground text-xs">Payment Method</span>
                          <p className="font-medium">{viewSheet.payment_method || "-"}</p>
                        </div>
                      </div>
                    </div>
                  </div>
