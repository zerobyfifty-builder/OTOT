...
export default function GodModeOverview() {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const {
        count: totalTrees,
        error: treesError
      } = await supabase.from('trees').select('*', {
        count: 'exact',
        head: true
      });
      ...
      const {
        data: treesData,
        error: treesDataError
      } = await supabase.from('trees').select('status, num_trees');
      ...
      const {
        ...
        error: transError
      } = await supabase.from('partner_transactions').select('amount');
      ...
      const {
        data: monthTransData,
        error: monthError
      } = await supabase.from('partner_transactions').select('amount').gte('created_at', thirtyDaysAgo.toISOString());
  ...
  if (loading) {
    return <div className="min-h-screen bg-admin-cream flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-admin-primary"></div>
      </div>;
  }
  return <div className="min-h-screen bg-admin-cream p-8">
      <div className="max-w-[1920px] mx-auto space-y-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-admin-primary mb-2">Dashboard</h1>
          <p className="text-admin-primary/70">Complete system oversight and control</p>
        </div>

        {/* Top Stats Row */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard title="Total Users" value={stats.totalUsers.toLocaleString()} icon={Users} description="Across all roles" trend={{
          value: stats.userGrowth,
          isPositive: true
        }} breakdown={[{
          label: 'Tourists',
          value: stats.userBreakdown.tourists
        }, {
          label: 'Partners',
          value: stats.userBreakdown.partners
        }, {
          label: 'Admins',
          value: stats.userBreakdown.admins
        }]} />

          <StatCard title="Total Trees" value={stats.totalTrees.toLocaleString()} icon={Trees} description={`${stats.totalCO2.toLocaleString()}kg CO₂ offset`} trend={{
          ...
          <StatCard title="Revenue" value={`$${stats.totalRevenue.toLocaleString()}`} icon={DollarSign} description={`$${stats.monthRevenue.toLocaleString()} this month`} trend={{
          value: stats.revenueGrowth,
          isPositive: true
        }} />

          <StatCard title="Active Sessions" value={stats.activeSessions} icon={ActivityIcon} description={`Peak today: ${stats.peakToday}`} breakdown={[{
          label: 'Tourists',
          value: Math.floor(stats.activeSessions * 0.7)
        }, {
          label: 'Partners',
          value: Math.floor(stats.activeSessions * 0.2)
        }, {
          label: 'Admins',
          value: Math.floor(stats.activeSessions * 0.1)
        }]} />
        </div>
