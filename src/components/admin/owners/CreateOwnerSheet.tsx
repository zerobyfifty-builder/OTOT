...
interface CreateOwnerSheetProps {
  open: boolean;
...
const emptyForm = {
  partnerTypeId: '', name: '', legalName: '', description: '',
  contactPerson: '', contactEmail: '', contactPhone: '',
  street: '', city: '', county: '', website: '', mouReference: '',
  userEmail: '', userPassword: '', userName: '',
};

export function CreateOwnerSheet({ open, onOpenChange, onCreated }: CreateOwnerSheetProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (form.userEmail && form.userPassword) {
        const { error: fnError } = await supabase.functions.invoke('create-owner-user', {
          body: {
      ...
      <Sheet open={open} onOpenChange={handleClose}>
        <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>Onboard a new owner organization and portal user.</SheetDescription>
          </SheetHeader>
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="flex justify-between pt-4">
                ...
                <Button onClick={() => setStep(2)} disabled={!form.partnerTypeId}>Next<ArrowRight className="h-4 w-4 ml-2" /></Button>
              </div>
            </div>
          )}
