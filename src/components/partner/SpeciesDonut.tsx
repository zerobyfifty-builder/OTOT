...
export function SpeciesDonut({ data, size = 180, thickness = 28 }: { data: SpeciesSlice[]; size?: number; thickness?: number }) {
  ...
  const slices = useMemo(() => {
    ...
    if (visible.length === 1) {
      return [{ ...visible[0], start: -Math.PI / 2, end: (3 * Math.PI) / 2 - 0.0001 }];
    }
    const gap = 0.04;
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="overflow-visible">
        {slices.map((s, i) => {
          ...
          return (
            <path
              key={s.name}
              d={arcPath(cx, cy, isActive ? rOuter + 4 : rOuter, rInner, s.start, s.end)}
              fill={s.color}
      ...
      {active && (
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none animate-fade-in">
