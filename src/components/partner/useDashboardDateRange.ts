...
import { startOfDay, startOfQuarter, startOfYear, subDays } from "date-fns";

export type DashboardPreset = "today" | "7d" | "30d" | "90d" | "quarter" | "ytd" | "all" | "custom";
...
const STORAGE_KEY = "partner-dashboard-range";
...
export function rangeFromPreset(preset: DashboardPreset, custom?: { from: Date; to: Date }): DashboardRange {
  ...
  switch (preset) {
    case "today":
      return { from: startOfDay(now), to, preset };
    case "7d":
      return { from: subDays(now, 7), to, preset };
    case "30d":
      return { from: subDays(now, 30), to, preset };
    case "90d":
      return { from: subDays(now, 90), to, preset };
    case "quarter":
      return { from: startOfQuarter(now), to, preset };
    case "ytd":
...
function load(): DashboardRange {
  try {
    ...
    if (!raw) return rangeFromPreset("all");
    ...
    return rangeFromPreset(parsed.preset || "all");
  } catch {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ preset: range.preset, from: range.from.toISOString(), to: range.to.toISOString() }),
      );
    } catch {
      /* ignore */
    }
